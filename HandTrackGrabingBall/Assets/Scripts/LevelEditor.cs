...
public class LevelEditor : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        // determine file paths for level and theme data
        string lPath = (levelRenderer != null && !string.IsNullOrEmpty(levelRenderer.jsonFilePath))
                        ? levelRenderer.jsonFilePath
                        : "LevelData/Level_wildhunt";

        string tPath = (themeChanger != null && !string.IsNullOrEmpty(themeChanger.jsonFilePath))
                        ? themeChanger.jsonFilePath
                        : "LevelData/Themes_wildhunt";

        // LOAD LEVEL DATA (prioritize persistent data)
        string persistentLevelPath = Path.Combine(Application.persistentDataPath, lPath + ".json");

        if (File.Exists(persistentLevelPath))
        {
            levelJsonString = File.ReadAllText(persistentLevelPath);
        }
        else
        {
            TextAsset asset = Resources.Load<TextAsset>(lPath);
            if (asset != null) levelJsonString = asset.text;
        }

        // Deserialize Level
        if (!string.IsNullOrEmpty(levelJsonString))
        {
            try
            {
        ...
        if (ldata == null)
        {
            Debug.LogWarning($"Can't find data for {lPath}. Create new empty data");
            ldata = new NewLevelJson();
            ldata.tiles = new List<List<int>>();
            ldata.enemies = new List<List<int>>();
            for (int i = 0; i < 10; i++)
            {
        ...
        // LOAD THEME DATA
        string persistentThemePath = Path.Combine(Application.persistentDataPath, tPath + ".json");

        if (File.Exists(persistentThemePath))
        {
            themeJsonString = File.ReadAllText(persistentThemePath);
        }
        else
        {
            TextAsset asset = Resources.Load<TextAsset>(tPath);
            if (asset != null) themeJsonString = asset.text;
        }

        if (!string.IsNullOrEmpty(themeJsonString))
        {
            try
            {
