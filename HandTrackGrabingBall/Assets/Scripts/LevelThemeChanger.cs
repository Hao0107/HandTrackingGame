...
public class LevelThemeChanger : MonoBehaviour
{
    ...
    private void Start()
    {
        ...
        // handle empty jsonFilePath 
        if (string.IsNullOrEmpty(jsonFilePath)) jsonFilePath = "LevelData/Themes_wildhunt";

        // Find file (Persistent -> Resources)
        ...
        else
        {
            ...
            {
                Debug.LogWarning($"[Theme] Can't find file: {jsonFilePath}. Create default theme.");
                // create default theme change event
                jsonString = "{ \"level_events\": [ { \"z_position\": 0.0, \"event_type\": \"theme_change\", \"event_fields\": { \"theme_id\": 0 } } ] }";
            }
        }
        ...
        try
        {
            ...
            if (jsonData != null && jsonData.level_events != null)
            {
                levelEvents = jsonData.level_events;

                // Set initial theme ID
            ...
            {
                levelEvents = new List<BaseEvent>();
            }
        }
        catch (Exception e)
        {
            Debug.LogError("ERROR reading Theme JSON: " + e.Message);
            levelEvents = new List<BaseEvent>();
        }
    }
